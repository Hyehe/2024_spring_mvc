...
public class SnSLoginController {
	...
	public ModelAndView kakaologin(HttpServletRequest request) {
		...
		try {
			...
			sb.append("&client_id=93006d28ac6f217d33503f6f0f67d192");
			sb.append("&redirect_uri=http://localhost:8080/kakaologin");
			sb.append("&code=" + code);
			sb.append("&client_secret=wYW8w8TcNz8DEE45M9QY2MWac6AZFpYC");
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 가져온 정보를 바탕으로 DB(-> id만 넣으면 됨)에 넣거나 session에 저장(둘 다 해도 됨)
				Gson gson = new Gson();
	...
	public ModelAndView naverlogin(HttpServletRequest request) {
		...
		try {
			...
			StringBuffer sb = new StringBuffer();
			sb.append("grant_type=authorization_code");
			sb.append("&client_id=Z0MDX6x3QjQ_HYJUJcKb");
			sb.append("&client_secret=rXO4XMcI7a");
			sb.append("&code=" + code);
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 해당 정보를 다른 Ajax 컨트롤러에서 사용하기 위해서 세션에 저장
	...
	public ModelAndView daumAddrOk(AddrVO addrvo, HttpSession session) {
		...
		// 로그인 성공한 상태에서 session 안에 있는 사용자 정보 중 m_id나 사용자테이블의 pk를 호출한다.
