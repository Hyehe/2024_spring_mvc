...
public class NaverUserInfoController {
	...
	public String NaverUserInfo(HttpServletRequest request) {
		...
		try {
			...
			conn.setRequestProperty("Authorization", "Bearer " + access_token);	// Bearer =>타입
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 네이버는 특별한 로그아웃이 없다. 그래서 로그아웃이 세션을 초기화한다.
				...
				// id를 가지고 사용자 DB에 검색해서 id가 없으면 DB에 등록한다.
