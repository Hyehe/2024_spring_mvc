...
public class ShopController {
	...
	public ModelAndView getShopShowCart(@ModelAttribute("shop_idx") String shop_idx,
		...
		try {
			...
			if (loginchk.equals("ok")) {
				...
				// 로그인한 정보를 가져온다.
				MemberVO mvo = (MemberVO) session.getAttribute("mvo2");  // session은 무조건 캐스팅하기
				
				// 제품 정보를 가져오자
				ShopVO svo = shopService.getShopDetail(shop_idx);
				
				// 카트리스트에 사용자의 m_id와 해당 제품 번호가 있는지(수량 1증가 update) 없는지(카트 추가 insert) 판별하기
				CartVO cartVO = shopService.getCartChk(mvo.getM_id(), svo.getP_num());
				
				if (cartVO == null) {
					// 카트가 비어있으므로 카트 테이블에 추가하기 (insert)
					CartVO cavo = new CartVO();
					cavo.setP_num(svo.getP_num());
					cavo.setP_name(svo.getP_name());
					cavo.setP_price(String.valueOf(svo.getP_price()));	 // int를 string으로 바꾸자
	...
	public ModelAndView getShopCartEdit(CartVO cavo) {
		try {
			...
			return null;  // null지정해둔 거 오류페이지 만들어서 가야되는 것들
		} catch (Exception e) {
			System.out.println(e);
			return null;
		}
